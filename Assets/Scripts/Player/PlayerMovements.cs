...
namespace Reconnect.Player
{
    public class PlayerMovements : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            _playerInput.actions["Dance"].started += OnDance;
        ...
        {
            if (_characterController.isGrounded && _isJumpingPressed) // jumping vertical velocity
            {
                _velocityY = Mathf.Sqrt(jumpHeight * -2f * gravity);
            }
            else if (_characterController.isGrounded) // on ground vertical velocity
            {
